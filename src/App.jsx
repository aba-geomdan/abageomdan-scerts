...
// =====================================================================
// Supabase 연결 설정 (env에서 읽음 — .env.local 또는 GitHub Secrets)
// =====================================================================
const SUPABASE_URL = import.meta.env?.VITE_SUPABASE_URL || '';
const SUPABASE_ANON_KEY = import.meta.env?.VITE_SUPABASE_ANON_KEY || '';
// 테이블: shared_store (shared=true), teacher_store (shared=false)
// 컬럼: key (text, PK), value (text), updated_at (timestamptz default now())

// =====================================================================
// 저장소 래퍼: window.storage와 동일 인터페이스로 Supabase REST API 호출
...
// =====================================================================
// 로그인 & 계정 관리 (공유 저장소 기반 — 로컬: localStorage / 다인용: window.storage 또는 Supabase)
// =====================================================================
const ADMIN_PW = 'abageomdan1121';  // 관리자 비밀번호
const ADMIN_NAME = '민다혜';          // 관리자 이름 (이 이름만 관리자 가능)
const MAX_USERS = 10;
const AUTH_KEY = 'gd-aba-auth';       // 로그인 세션 (기기별, shared=false)
const USERS_KEY = 'gd-aba-users';     // 선생님 계정 목록 (공유, shared=true)
const STATE_PREFIX = 'gd-aba-state:'; // 선생님별 작업 데이터 (공유, shared=true)
const ARCHIVE_PREFIX = 'gd-aba-archive:'; // 선생님별 보관함 (공유, shared=true)
...
export default function App() {
  // ── 로그인 상태 ──
  const [authUser, setAuthUser] = useState(null);     // {name, role:'admin'|'therapist'}
  const [authLoading, setAuthLoading] = useState(true);
  const [loginName, setLoginName] = useState('');
  const [loginPw, setLoginPw] = useState('');
  const [loginError, setLoginError] = useState('');
  const [userList, setUserList] = useState([]);       // 선생님 계정 목록

  // 앱 시작 시: 세션 복원 + 사용자 목록 로드
  useEffect(() => {
    (async () => {
      try {
        if (!window.storage) { setAuthLoading(false); return; }
        try {
          const sess = await window.storage.get(AUTH_KEY, false);
          if (sess?.value) { const u = JSON.parse(sess.value); if (u.name) setAuthUser(u); }
        } catch (e) {}
        try {
          const ul = await window.storage.get(USERS_KEY, true);
          if (ul?.value) setUserList(JSON.parse(ul.value));
        } catch (e) {}
      } catch (e) {}
  ...
  const saveUsers = async (list) => {
    setUserList(list);
    try { if (window.storage) await window.storage.set(USERS_KEY, JSON.stringify(list), true); } catch (e) {}
  };

  const handleLogin = async () => {
    if (!loginName.trim() || !loginPw.trim()) { setLoginError('이름과 비밀번호를 입력하세요.'); return; }
    // 관리자: 지정된 이름 + 관리자 비번
    if (loginName.trim() === ADMIN_NAME && loginPw.trim() === ADMIN_PW) {
      const session = { name: ADMIN_NAME, role: 'admin' };
    ...
    if (loginPw.trim() === ADMIN_PW && loginName.trim() !== ADMIN_NAME) {
  ...
  const handleLogout = async () => {
    setAuthUser(null);
    setLoginName(''); setLoginPw(''); setLoginError('');
    try { if (window.storage) await window.storage.delete(AUTH_KEY, false); } catch (e) {}
  };

  // ── 계정 관리 (관리자 전용) ──
  const addUser = async (name, pw) => {
    const nm = (name || '').trim(), pwd = (pw || '').trim();
  ...
  return <AppInner
    ...
    userList={userList} saveUsers={saveUsers}
    addUser={addUser} removeUser={removeUser} changeUserPw={changeUserPw}
  />;
}

// 실제 앱 본체 (로그인 통과 후)
function AppInner({ authUser, authLoading, loginName, setLoginName, loginPw, setLoginPw, loginError, handleLogin, handleLogout, userList, saveUsers, addUser, removeUser, changeUserPw }) {
  ...
  useEffect(() => {
    ...
    const check = async () => {
      try {
        const r = await window.storage.get(STATE_PREFIX + safeKeyName(authUser.name), true);
        if (r?.value) {
  ...
  // 관리자: 모든 선생님의 데이터를 공유 저장소에서 수집
  const loadAllTeacherData = useCallback(async () => {
    if (!window.storage) { setAllTeacherData([]); return; }
    setAdminLoading(true);
    try {
      const res = await window.storage.list(STATE_PREFIX, true);
      const keys = (res && res.keys) || [];
      const collected = [];
      for (const k of keys) {
        try {
          const teacherName = k.replace(STATE_PREFIX, '');
          ...
          collected.push({ teacher: teacherName, childCount: children.length, children, snapshot: st });
        } catch (e) {}
      }
      // 관리자 본인 데이터도 포함 (현재 state)
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        const key = STATE_PREFIX + safeKeyName(authUser.name);
      ...
      try {
        const akey = ARCHIVE_PREFIX + safeKeyName(authUser.name);
    ...
    const t = setTimeout(() => {
      try {
        ...
        if (window.storage && authUser?.name) {
          window.storage.set(STATE_PREFIX + safeKeyName(authUser.name), JSON.stringify(stamped), true).catch(() => {});
        }
        const now = Date.now();
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (window.storage && authUser?.name) {
        window.storage.set(ARCHIVE_PREFIX + safeKeyName(authUser.name), JSON.stringify(archive), true).catch(() => {});
      }
    } catch (e) {}
      ...
      <div className="login-screen">
        ...
        <div className="login-box">
          ...
          <div className="login-fields">
            ...
            <input className="login-input" type="text" value={loginName}
              onChange={(e) => setLoginName(e.target.value)}
              onKeyDown={(e) => { if (e.key === 'Enter') handleLogin(); }}
              placeholder="선생님 이름" autoFocus
              autoComplete="off"
              name={`scerts-login-name-${Math.random().toString(36).slice(2, 8)}`}
              spellCheck={false}
              autoCorrect="off"
              autoCapitalize="off" />
          ...
          <div className="login-hint">
            관리자는 지정된 이름과 관리자 비밀번호로 로그인하세요.<br />
            선생님 계정은 관리자가 발급합니다.
          </div>
...
function AccountPanel({ userList, addUser, removeUser, changeUserPw, onClose }) {
  ...
  const [editPwFor, setEditPwFor] = useState(null);
  const [editPwVal, setEditPwVal] = useState('');
  const [confirmDel, setConfirmDel] = useState(null);

  const handleAdd = async () => {
    const r = await addUser(newName, newPw);
    if (r.ok) { setNewName(''); setNewPw(''); setMsg(`✅ "${newName.trim()}" 선생님 계정이 생성되었습니다.`); }
    else setMsg('⚠️ ' + r.msg);
  };
  const handleChangePw = async (name) => {
    const r = await changeUserPw(name, editPwVal);
    if (r.ok) { setMsg(`✅ "${name}" 선생님 비밀번호가 변경되었습니다. 새 비밀번호: ${editPwVal.trim()}`); setEditPwFor(null); setEditPwVal(''); }
    else setMsg('⚠️ ' + r.msg);
  };
    ...
    <div className="app-dialog-overlay no-print" onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="app-dialog account-panel" role="dialog" aria-modal="true">
        <div className="account-panel-head">
          ...
          <span className="account-panel-count">{userList.length}/{MAX_USERS}명</span>
        </div>
        ...
        <div className="account-add">
          ...
          <div className="account-add-row">
            <input className="account-input" type="text" value={newName}
              onChange={(e) => setNewName(e.target.value)} placeholder="선생님 이름"
            ...
            <input className="account-input" type="text" value={newPw}
              onChange={(e) => setNewPw(e.target.value)} placeholder="비밀번호 지정"
          ...
          <div className="account-add-hint">이름과 비밀번호를 정해 선생님께 알려주세요. 선생님은 이 정보로 로그인합니다.</div>
        </div>

        {msg && <div className="account-msg">{msg}</div>}

        <div className="account-list">
          {userList.length === 0 && <div className="account-empty">아직 등록된 선생님이 없습니다.</div>}
          {userList.map((u) => (
              ...
              {editPwFor === u.name ? (
                <div className="account-item-edit">
                  <input className="account-input account-input-sm" type="text" value={editPwVal}
                    ...
                    onKeyDown={(e) => { if (e.key === 'Enter') handleChangePw(u.name); }} autoFocus />
                  <button className="btn-primary btn-small" onClick={() => handleChangePw(u.name)}>저장</button>
                  <button className="btn-ghost btn-small" onClick={() => { setEditPwFor(null); setEditPwVal(''); }}>취소</button>
                </div>
              ) : confirmDel === u.name ? (
                <div className="account-item-edit">
                  <span className="account-del-warn">삭제할까요? (아동 데이터는 보존됨)</span>
                  <button className="btn-danger btn-small" onClick={async () => { await removeUser(u.name); setConfirmDel(null); setMsg(`🗑️ "${u.name}" 계정이 삭제되었습니다.`); }}>삭제</button>
                  <button className="btn-ghost btn-small" onClick={() => setConfirmDel(null)}>취소</button>
                </div>
              ) : (
                <div className="account-item-actions">
                  <button className="btn-ghost btn-small" onClick={() => { setEditPwFor(u.name); setEditPwVal(''); }}>비번 변경</button>
                  <button className="btn-ghost btn-small account-del-btn" onClick={() => setConfirmDel(u.name)}>삭제</button>
                </div>
              )}
            </div>
          ))}
